...
function isPlainObject(o) {
  ...
  if (!Object.hasOwn(prot, "isPrototypeOf")) return false;
...
function ensureQueryFn(options, fetchOptions) {
  ...
  if (!options.queryFn || options.queryFn === skipToken)
    return () => Promise.reject(/* @__PURE__ */ new Error(`Missing queryFn: '${options.queryHash}'`));
...
function dehydrate(client, options = {}) {
  ...
  return {
    ...
    queries: client
      ...
      .getAll()
      .flatMap((query) =>
        filterQuery(query) ? [dehydrateQuery(query, serializeData, shouldRedactErrors)] : []
      ),
  };
}
  ...
  queries.forEach(({ queryKey, state, queryHash, meta, promise, dehydratedAt }) => {
    ...
    if (query) {
      const hasNewerSyncData =
        syncData && dehydratedAt !== void 0 && dehydratedAt > query.state.dataUpdatedAt;
    ...
    if (
      ...
      query.fetch(void 0, { initialPromise: Promise.resolve(promise).then(deserializeData) }).catch(noop);
  });
}
  ...
  removeObserver(observer) {
    if (this.observers.includes(observer)) {
      ...
      if (!this.observers.length) {
        if (this.#retryer)
          if (this.#abortSignalConsumed) this.#retryer.cancel({ revert: true });
