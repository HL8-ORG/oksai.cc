...
var _handleNotFound = (inner, err, routerCode) => {
	...
	if (!willWalkUp) invariant(routeCursor.options.notFoundComponent, "No notFoundComponent found. Please set a notFoundComponent on your route or provide a defaultNotFoundComponent to the router.");
...
var shouldSkipLoader = (inner, matchId) => {
	if (inner.router.getMatch(matchId).ssr === false) return true;
	return false;
};
var handleSerialError = (inner, index, err, routerCode) => {
	...
	handleRedirectAndNotFound(inner, inner.router.getMatch(matchId), err, routerCode);
	try {
...
var loadRouteMatch = async (inner, index) => {
	async function handleLoader(preload, prevMatch, match2, route2) {
		...
		} else if (status !== "success" || loaderShouldRunAsync && inner.sync) await runLoader(inner, matchId, index, route2);
	}
	const { id: matchId, routeId } = inner.matches[index];
	let loaderShouldRunAsync = false;
	let loaderIsRunningAsync = false;
	const route = inner.router.looseRoutesById[routeId];
	if (shouldSkipLoader(inner, matchId)) return inner.router.getMatch(matchId);
	else {
		...
		if (prevMatch._nonReactive.loaderPromise) {
			...
			if (match2.status === "pending") await handleLoader(preload, prevMatch, match2, route);
		} else {
			...
			const parse = ({ pathname, search, hash, href, state }) => {
				if (!this.rewrite && !/[ \x00-\x1f\x7f\u0080-\uffff]/.test(pathname)) {
					...
					return {
						...
						search: replaceEqualDeep(previousLocation?.search, parsedSearch2),
			...
			loadPromise = new Promise((resolve) => {
				this.startTransition(async () => {
					try {
						...
						if (isRedirect(err)) redirect2 = err;
	...
	const { activeProps, inactiveProps, activeOptions, to, preload: userPreload, preloadDelay: userPreloadDelay, hashScrollIntoView, replace, startTransition, resetScroll, viewTransition, children, target, disabled, style, className, onClick, onBlur, onFocus, onMouseEnter, onMouseLeave, onTouchStart, ignoreBlocker, params: _params, search: _search, hash: _hash, state: _state, mask: _mask, reloadDocument: _reloadDocument, unsafeRelative: _unsafeRelative, from: _from, _fromLocation, ...propsSafeToSpread } = options;
	{
		...
		const externalLink2 = (() => {
			if (hrefOption2?.external) {
				if (isDangerousProtocol(hrefOption2.href, router.protocolAllowlist)) return;
