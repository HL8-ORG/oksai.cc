...
export const Guard = {
   ...
   */
  isNegativeNumber(value: unknown): boolean {
    return typeof value === "number" && !Number.isNaN(value) && value < 0;
  },
   ...
   * @returns 验证结果
   */
  againstEmpty(field: string, value: unknown): Result<void, GuardError> {
    if (Guard.isEmpty(value)) {
      return Result.fail({
        field,
        message: `${field} 不能为空`,
   ...
   * @returns 验证结果
   */
  againstNull(field: string, value: unknown): Result<void, GuardError> {
    if (value === null) {
      return Result.fail({
        field,
   ...
   * @returns 验证结果
   */
  againstUndefined(field: string, value: unknown): Result<void, GuardError> {
    if (value === undefined) {
      return Result.fail({
        field,
   ...
   * @returns 组合后的验证结果
   */
  combine(results: Result<void, GuardError>[]): Result<void, GuardError[]> {
