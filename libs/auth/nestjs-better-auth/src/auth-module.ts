...
export class AuthModule extends ConfigurableModuleClass implements NestModule, OnModuleInit {
  ...
  private setupHooks(
    ...
    for (const { metadataKey, hookType } of HOOKS) {
      ...
      this.options.auth.options.hooks[hookType] = createAuthMiddleware(async (ctx: AuthHookContext) => {
        if (originalHook) {
  ...
  static forRoot(arg1: Auth | typeof OPTIONS_TYPE, arg2?: Omit<typeof OPTIONS_TYPE, "auth">): DynamicModule {
